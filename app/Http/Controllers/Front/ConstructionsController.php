...
class ConstructionsController extends FrontController {
    ...
    {
        $records = Company::latest()->where('service_id','1')->get();
...
//            $record->properties = \DB::table('properties')->where('user_id', $record->id)->get()->count();
        ...
        try {
            return view('front.constructions.index', ['records' => $records,
